...
public class RateUtils {
    ...
    public static void insertRate(final AnalysisResult analysisResult, final Map<String, Integer> ratesValues, String rate) {
        ...
        if (parts.length > 1) {
            analysisResult.getRateObjects().add(new RateObject(ratesValues.get(rate), parts[0], parts[1], 0, 0, 0, 0));
        } else {
            analysisResult.getRateObjects().add(new RateObject(ratesValues.get(rate), rate, null, 0, 0, 0, 0));
        }
    }
